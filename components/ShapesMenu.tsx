  ...
  return (
    <>
      <DropdownMenu>
        ...
        <DropdownMenuContent className="mt-5 flex flex-col brn gap-y-1 border-none bg-primary-black py-4">
          {item.value.map((elem) => (
            <Button
              key={elem?.name}
              onClick={() => {
              ...
              className={`flex h-fit justify-between gap-10 brn rounded-none px-5 py-3 focus:border-none ${
                activeElement.value === elem?.value ? "brn" : "hover:bg-white"
              }`}
            >
              <div className="group flex items-center gap-2">
                ...
                <p
                  className={`text-sm  ${
                    activeElement.value === elem?.value ? "text-primary-black" : ""
                  }`}
                >
                  {elem?.name}
                </p>
